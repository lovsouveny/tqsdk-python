...
class TqSim(object):
            ...
            async for pack in self._api_send_chan:
                ...
                if "_md_recv" in pack:
                    ...
                    if symbol not in self._quote_tasks:
                        ...
                        settle_chan = TqChan(self._api)
                        self._quote_tasks[symbol] = {
                            "quote_chan": quote_chan,
                            "order_chan": order_chan,
                            "settle_chan": settle_chan,
                            "task": self._api.create_task(self._quote_handler(symbol, quote_chan, order_chan, settle_chan))
                        }
    ...
    async def _quote_handler(self, symbol, quote_chan, order_chan, settle_chan):
        orders = {}
    ...
    def _md_recv(self, pack):
        for d in pack["data"]:
            ...
            # 保证在需要结算的时候，quote_chan 里先发送 settle，后发送行情
            for symbol, quote_diff in quotes_diff.items():
                if quote_diff is None:
                    continue
                if not self._tqsdk_backtest:
                    dt = quote_diff.get("datetime", "")
                    self._current_datetime = dt if dt > self._current_datetime else self._current_datetime  # 最新行情时间
                ...
                if self._current_datetime > self._trading_day_end:  # 结算
                    ...
                    for quote_task in self._quote_tasks.values():
                        quote_task["quote_chan"].send_nowait({"aid": "settle"})
